...
const treeJson = readFileSync(getFixturePath('formatJson.txt'), 'utf-8');
...
test(('json'), () => {
  expect(genDiff(getFixturePath('file1.json'), getFixturePath('file2.json'), 'json')).toBe(treeJson);
  expect(genDiff(getFixturePath('file1.yaml'), getFixturePath('file2.yaml'), 'json')).toEqual(treeJson);
});
